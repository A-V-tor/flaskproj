...
class TestView:
    ...
    def test_registration(self):
        ...
        response = self.client.post(
            "/registration",
            data={"name": "admin48", "psw": "admin48", "email": "admin48@bail.com"},
    ...
    def test_autorization(self):
        ...
        response = self.client.post(
            "/autorization",
            data={"name": "adminnnn", "psw": "adminnnn"},
            follow_redirects=True,
        )
        html = response.get_data(as_text=True)
        assert "Неверное имя или пароль!" in html
        response = self.client.post(
            "/autorization", data={"name": "", "psw": "adminnnn"}, follow_redirects=True
        )
        html = response.get_data(as_text=True)
